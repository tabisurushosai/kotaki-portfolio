...
const MIRAI_FEATURES: ProjectFeature[] = [
  ...
  {
    icon: ClipboardList,
    title: "フリースクール出席管理ツール開発",
    teaser:
      "日次入力から学校提出書類の出力まで、1画面で完結する業務ツール",
    body: "フリースクール教員向けに、出席・授業内容・月次所見の入力から学校提出用Excel・日本語PDFまでを1画面で完結させるWebツールを設計・実装。ワンタップの教科選択と自動保存で入力負荷と保存漏れを抑え、既存フォーマットに沿った帳票はワンクリック生成。受講数などの集計は自動。出力Excelを再アップロードすると差分のみGoogle Sheetsへ反映し、手元での一括編集やバックアップ運用にもつなげています。",
  },
];
    ...
    <div className="works-root flex flex-col gap-8">
      ...
      <div className="works-project-grid grid gap-6" id="works-grid">
        ...
        <ProjectCard
          ...
          subtitle="戦略支援、体験型イベントの壁打ち、フリースクール業務ツール"
          description="地域の子どもと家族に届く教育・体験事業を支えるMiRAI様向けに、競合分析から戦略会議の参謀、体験型イベント企画の対話型アシスタントを設計・実装。あわせて、フリースクールで日次入力から学校提出用帳票（Excel/PDF）までを1画面で完結する業務ツールを構築し、手入力運用の体裁崩れと工数を減らしています。"
